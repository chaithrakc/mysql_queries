...
time_stamp and that confirmation message was either confirmed ('confirmed') or expired without 
confirming ('timeout').
...
select user_id, 
round(
    ifnull( sum(action='confirmed')/count(action) ,0)
    , 2) as confirmation_rate
from signups
left join confirmations using (user_id)
group by user_id;

-- filtering the records inside sum() function instead of having clause


/*
{"headers": {"Signups": ["user_id", "time_stamp"], 
"Confirmations": ["user_id", "time_stamp", "action"]}, 
"rows": {"Signups": [[3, "2020-03-21 10:16:13"], 
                    [7, "2020-01-04 13:57:59"], 
                    [2, "2020-07-29 23:09:44"], 
                    [6, "2020-12-09 10:39:37"]], 
        "Confirmations": [[3, "2021-01-06 03:30:46", "timeout"], 
                        [3, "2021-07-14 14:00:00", "timeout"], 
                        [7, "2021-06-12 11:57:29", "confirmed"], 
                        [7, "2021-06-13 12:58:28", "confirmed"], 
                        [7, "2021-06-14 13:59:27", "confirmed"], 
