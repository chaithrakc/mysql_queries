...
from orders ord inner join product pd on ord.product_id = pd.product_id
inner join customers cust on ord.customer_id = cust.customer_id
group by cust.customer_id, cust.name
having sum(quantity*price*(month(order_date)=6)) >=100
    and sum(quantity*price*(month(order_date)=7)) >= 100;

/*
input:
{"headers": {"Customers": ["customer_id", "name", "country"], 
            "Product": ["product_id", "description", "price"], 
            "Orders": ["order_id", "customer_id", "product_id", "order_date", "quantity"]}, 

"rows": {"Customers": [[1, "Winston", "USA"], 
                       [2, "Jonathan", "Peru"], 
                       [3, "Moustafa", "Egypt"]], 
        "Product": [[10, "LC Phone", 300], 
                    [20, "LC T-Shirt", 10], 
                    [30, "LC Book", 45], 
                    [40, "LC Keychain", 2]], 
        "Orders": [ [1, 1, 10, "2020-06-10", 1], 
                    [2, 1, 20, "2020-07-01", 1], 
                    [3, 1, 30, "2020-07-08", 2], 
                    [4, 2, 10, "2020-06-15", 2], 
                    [5, 2, 40, "2020-07-01", 10], 
...
output:
{"headers": ["customer_id", "name"], "values": [[1, "Winston"]]}
