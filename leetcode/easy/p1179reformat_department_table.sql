...
--using partition by: redundant to generate rownum becuase table already has id 
select id, 
sum(if(month='Jan', revenue,null)) as Jan_Revenue,
sum(if(month='Feb', revenue,null)) as Feb_Revenue,
sum(if(month='Mar', revenue,null)) as Mar_Revenue,
...
sum(if(month='Nov', revenue,null)) as Nov_Revenue,
sum(if(month='Dec', revenue,null)) as Dec_Revenue
from (select id, revenue, month, row_number() over (partition by month) as rownum
...
-- without common table expression also works
select id, 
"Jan" as "Jan_Revenue", 
"Feb" as "Feb_Revenue", 
"Mar" as "Mar_Revenue", 
